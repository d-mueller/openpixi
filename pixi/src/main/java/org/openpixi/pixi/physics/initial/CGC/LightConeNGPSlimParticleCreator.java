 ...
 */
public class LightConeNGPSlimParticleCreator implements IParticleCreator {
	...
	public void initializeParticles(Simulation s, int particlesPerLink) {
		...
		for (int i = 0; i < s.grid.getTotalNumberOfCells(); i++) {
			for (int j = 0; j < particlesPerCell; j++) {
				...
				for (int k = 0; k < s.getNumberOfDimensions(); k++) {
					p.setPosition(k, particlePosition1[k]);
					p.setPrevPosition(k, particlePosition0[k]);
					p.setVelocity(k, particleVelocity[k]);
					p.Q0 = charge;
					p.Q1 = charge.copy();
				}
